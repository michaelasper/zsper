...
def test_air_profile_defaults_match_spec(tmp_path: Path) -> None:
    ...
    assert profile.mode == "air-offline"
    ...
    assert profile.database_name == "zsper_air_offline"
...
def test_initialize_air_profile_writes_profile_layout_and_registry(
    ...
    assert profile_json["mode"] == "air-offline"
...
def test_resolve_and_doctor_air_profile(
    ...
    assert loaded.mode == "air-offline"
    assert report.ok is True
    assert report.profile.name == "air"
    assert report.errors == []


def test_air_profile_invariants_reject_local_first_network(tmp_path: Path) -> None:
